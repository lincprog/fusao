...
def crawl(id, qtdPaginas, qtdItens, nome, pular):
	...
	for i in range(qtdPaginas):	# For each page:
		...
		req = urllib.request.Request(url='https://iosearch.reclameaqui.com.br/raichu-io-site-search-v1/query/companyComplains/'+str(qtdItens)+'/'+str(qtdItens * (i+pular))+'?company='+str(id),#+'&evaluated=bool:true',
									 origin_req_host="https://www.reclameaqui.com.br",
									 headers={
									'User-Agent': ' Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36',
									'Referer': 'https://www.reclameaqui.com.br/empresa/'+nome+'/lista-reclamacoes/?pagina='+str(i+pular+1)
		...
		reclamacoes = pagina_json["complainResult"]["complains"]["data"] # here lay our treasures
		...
		for reclamacao in reclamacoes:
			...
			rec_req = urllib.request.Request(url='https://iosite.reclameaqui.com.br/raichu-io-site-v1/complain/public/'+hash,
							 origin_req_host="https://www.reclameaqui.com.br",
							 headers={
							'User-Agent': ' Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36',
							'Referer': 'https://www.reclameaqui.com.br/empresa/'+nome+'/lista-reclamacoes/?pagina='+str(i+pular+1)
			...
			companyreply 	= "-"
			userreply 		= "-"
			if "interactions" in rec_pagina_json.keys() and len(rec_pagina_json["interactions"]) >= 1 : # if there are interactions
				interactions = list(filter(lambda x: not x["deleted"], rec_pagina_json["interactions"])) # remove the deleted messages
				interactions = list(map(lambda x: {"message": x["message"], "type": x["type"]}, interactions)) # clean not-used key-value pairs
				# now we have our filtered-and-mapped interactions
				answers = list(filter(lambda x: x["type"]=="ANSWER", interactions)) # For all the companie answers:
				if answers : companyreply = answers[0]["message"] # take the first one
			...
			# the above line can be omitted
				replys = list(filter(lambda x: x["type"]=="REPLY", interactions)) # For all the user replys:
